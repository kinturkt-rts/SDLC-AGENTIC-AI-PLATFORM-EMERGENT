...
#
#   .\scripts\deploy-target-app.ps1 -Feature hello-fastapi              # build + push + apply
#   .\scripts\deploy-target-app.ps1 -Feature hello-fastapi -SkipBuild   # infra only
#   .\scripts\deploy-target-app.ps1 -Feature hello-fastapi -PlanOnly    # terraform plan, no changes
#   .\scripts\deploy-target-app.ps1 -Feature hello-fastapi -Destroy     # tear everything down
#
# Prereqs (one-time, already provisioned):
#   - infrastructure/bootstrap        -> S3 state bucket
#   - infrastructure/environments/dev/_shared -> ECS cluster + shared ALB
# Per-app prereq: infrastructure/environments/dev/<Feature>/ Terraform root
# (hand-written for hello-fastapi; generated by devops-agent in Phase B).
...
if (-not $env:TF_VAR_database_url -and $env:POSTGRES_MCP_DB_PASSWORD -and $env:POSTGRES_MCP_DB_ENDPOINT -and $env:POSTGRES_MCP_DATABASE -and $env:POSTGRES_MCP_DB_USER) {
    ...
    $env:TF_VAR_database_url = "postgresql://$($env:POSTGRES_MCP_DB_USER):$($env:POSTGRES_MCP_DB_PASSWORD)@$($env:POSTGRES_MCP_DB_ENDPOINT):$dbPort/$($env:POSTGRES_MCP_DATABASE)"
