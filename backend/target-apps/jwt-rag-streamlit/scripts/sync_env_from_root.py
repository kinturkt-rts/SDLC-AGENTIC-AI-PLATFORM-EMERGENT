"""Populate target-apps/jwt-rag-streamlit/.env from the project root .env."""
...
PLACEHOLDER_JWT = "your-secret-key-change-in-production"
...
def update_app_env(app_env_path: Path, database_url: str) -> list[str]:
    ...
    for line in app_env_path.read_text(encoding="utf-8").splitlines():
        ...
        if stripped.startswith("JWT_SECRET_KEY=") and PLACEHOLDER_JWT in stripped:
            new_lines.append(f"JWT_SECRET_KEY=jrs-{secrets.token_hex(24)}")
