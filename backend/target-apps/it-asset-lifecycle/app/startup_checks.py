...
def validate_runtime_config(settings: "Settings") -> None:
    ...
    if postgres_app and not _is_postgres_url(url):
        ...
        if shell_val and shell_val.startswith("sqlite") and _is_postgres_url(dotenv_val):
            hint = (
                ...
                f"environment variable DATABASE_URL='{shell_val[:60]}' is overriding it. ***\n"
    ...
    # Fail fast if ASSET_ENCRYPTION_KEY or JWT_SECRET are missing
    if not (settings.asset_encryption_key or "").strip():
        raise RuntimeError(
            "ASSET_ENCRYPTION_KEY is not set. Generate a Fernet key:\n"
            "  python -c \"from cryptography.fernet import Fernet; print(Fernet.generate_key().decode())\""
        )
    if not (settings.jwt_secret or "").strip():
        raise RuntimeError(
            "JWT_SECRET is not set. Add a strong random string to .env:\n"
