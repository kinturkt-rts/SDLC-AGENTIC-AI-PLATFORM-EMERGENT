# Database handoff — bug-deduper

_Generated 2026-06-22 15:18 UTC by database-agent._
...
| Item | Value |
|------|--------|
| Postgres schema | `bug_deduper` |
| Database | `sdlc_agentic_ai` |
| Endpoint | `agenticaidbinstance.c1u0cggiolxp.us-east-2.rds.amazonaws.com` |
| SQL artifacts | `target-apps/bug-deduper/db/sql/` |
| RDS apply (last run) | not this session |
| Dev seed rows/table | 5–10 (see `*_seed.sql`) |

## SQL files (apply order)

1. `target-apps/bug-deduper/db/sql/001_enable_pgvector.sql`
2. `target-apps/bug-deduper/db/sql/002_create_enums.sql`
3. `target-apps/bug-deduper/db/sql/003_create_bugs.sql`
4. `target-apps/bug-deduper/db/sql/004_create_api_keys.sql`
5. `target-apps/bug-deduper/db/sql/011_seed.sql`

**Connection:** load credentials from env/Key Vault (NFR-5). Use schema `bug_deduper` (`search_path` or qualified table names). Do not rely on unqualified `public` for app tables.

## ORM parity (required for live RDS)

- **ENUM `bug_status`** → `sqlalchemy.Enum(..., name='bug_status', schema='bug_deduper', create_type=False, native_enum=True)` + `.with_variant(String, 'sqlite')` (see `_template/app/models/pg_types.py`).
- **ENUM `key_tier`** → `sqlalchemy.Enum(..., name='key_tier', schema='bug_deduper', create_type=False, native_enum=True)` + `.with_variant(String, 'sqlite')` (see `_template/app/models/pg_types.py`).
...
- **Driver/DSN** → `psycopg[binary]` in requirements; `.env.example`: `postgresql+psycopg://...?sslmode=require`; `POSTGRES_SCHEMA=bug_deduper` (set search_path in `database.py`, not copied from other apps).

## Developer-agent checklist

1. `dev_read_file` → `docs/design/bug-deduper.md`
2. `dev_read_file` → `target-apps/bug-deduper/db/sql/` migrations + seed
