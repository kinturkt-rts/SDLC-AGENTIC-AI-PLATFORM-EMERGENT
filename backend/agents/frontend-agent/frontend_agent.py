...
- Store the auth token via the TOKEN_KEY constant exported from api.ts — import { TOKEN_KEY } from './api' and call localStorage.setItem(TOKEN_KEY, response.access_token) on login, localStorage.removeItem(TOKEN_KEY) on logout. NEVER hardcode the literal string "token" — TOKEN_KEY is namespaced per app (not the bare string "token") and the api helpers only read the real TOKEN_KEY value, so hardcoding "token" directly breaks authentication.
