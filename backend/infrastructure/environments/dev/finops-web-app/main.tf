# Per-app deploy root for target-app: finops-web-app
# Auto-scaffolded by scripts/ensure-target-app-tf-root.py for destroy/redeploy.
# Deploy with: .\scripts\deploy-target-app.ps1 -Feature finops-web-app
...
terraform {
  ...
  backend "s3" {
    ...
    key          = "dev/apps/finops-web-app/terraform.tfstate"
    region       = "us-east-2"
    use_lockfile = true
  }
...
resource "aws_secretsmanager_secret" "db" {
  name                    = "sdlc/finops-web-app/database-url"
...
output "app_url" {
  value = "http://${data.terraform_remote_state.shared.outputs.alb_dns_name}/finops-web-app/"
}
