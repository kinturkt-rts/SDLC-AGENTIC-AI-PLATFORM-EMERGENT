# Per-app deploy root for target-app: benefits-qa-desk (generated by devops-agent).
# Deploy with: .\scripts\deploy-target-app.ps1 -Feature benefits-qa-desk
...
terraform {
  ...
  backend "s3" {
    ...
    key          = "dev/apps/benefits-qa-desk/terraform.tfstate"
    region       = "us-east-2"
    use_lockfile = true
  }
...
module "app" {
  ...
  enable_bedrock = true
  extra_env = {
    JWT_SECRET_KEY = "l1rGJQYi1jOz9rck0OI97EYBLZ4z_m1B"
  }
  has_database         = true
...
resource "aws_secretsmanager_secret" "db" {
  name                    = "sdlc/benefits-qa-desk/database-url"
...
output "app_url" {
  value = "http://${data.terraform_remote_state.shared.outputs.alb_dns_name}/benefits-qa-desk/"
}
