# Per-app deploy root for target-app: expense-tracker (generated by devops-agent).
# Deploy with: .\scripts\deploy-target-app.ps1 -Feature expense-tracker
...
terraform {
  ...
  backend "s3" {
    ...
    key          = "dev/apps/expense-tracker/terraform.tfstate"
    region       = "us-east-2"
    use_lockfile = true
  }
...
resource "aws_secretsmanager_secret" "db" {
  name                    = "sdlc/expense-tracker/database-url"
...
output "app_url" {
  value = "http://${data.terraform_remote_state.shared.outputs.alb_dns_name}/expense-tracker/"
}
