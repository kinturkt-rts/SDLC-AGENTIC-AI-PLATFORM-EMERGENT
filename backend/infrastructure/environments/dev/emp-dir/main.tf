# Per-app deploy root for target-app: emp-dir
# Auto-scaffolded by scripts/ensure-target-app-tf-root.py for destroy/redeploy.
# Deploy with: .\scripts\deploy-target-app.ps1 -Feature emp-dir
...
terraform {
  ...
  backend "s3" {
    ...
    key          = "dev/apps/emp-dir/terraform.tfstate"
    region       = "us-east-2"
    use_lockfile = true
  }
...
resource "aws_secretsmanager_secret" "db" {
  name                    = "sdlc/emp-dir/database-url"
...
output "app_url" {
  value = "http://${data.terraform_remote_state.shared.outputs.alb_dns_name}/emp-dir/"
}
