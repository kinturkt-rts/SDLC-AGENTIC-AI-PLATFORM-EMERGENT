# Per-app deploy root for target-app: hotel-room
# Auto-scaffolded by scripts/ensure-target-app-tf-root.py for destroy/redeploy.
# Deploy with: .\scripts\deploy-target-app.ps1 -Feature hotel-room
...
terraform {
  ...
  backend "s3" {
    ...
    key          = "dev/apps/hotel-room/terraform.tfstate"
    region       = "us-east-2"
    use_lockfile = true
  }
...
resource "aws_secretsmanager_secret" "db" {
  name                    = "sdlc/hotel-room/database-url"
...
output "app_url" {
  value = "http://${data.terraform_remote_state.shared.outputs.alb_dns_name}/hotel-room/"
}
