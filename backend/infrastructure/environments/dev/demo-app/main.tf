# Per-app deploy root for target-app: demo-app (generated by devops-agent).
# Deploy with: .\scripts\deploy-target-app.ps1 -Feature demo-app
...
terraform {
  ...
  backend "s3" {
    ...
    key          = "dev/apps/demo-app/terraform.tfstate"
    region       = "us-east-2"
    use_lockfile = true
  }
...
output "app_url" {
  value = "http://${data.terraform_remote_state.shared.outputs.alb_dns_name}/demo-app/"
}
