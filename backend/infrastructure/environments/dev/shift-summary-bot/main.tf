# Per-app deploy root for target-app: shift-summary-bot
# Destroy with: .\scripts\deploy-target-app.ps1 -Feature shift-summary-bot -Destroy
...
terraform {
  ...
  backend "s3" {
    ...
    key          = "dev/apps/shift-summary-bot/terraform.tfstate"
    region       = "us-east-2"
    use_lockfile = true
  }
...
resource "aws_secretsmanager_secret" "db" {
  name                    = "sdlc/shift-summary-bot/database-url"
...
output "app_url" {
  value = "http://${data.terraform_remote_state.shared.outputs.alb_dns_name}/shift-summary-bot/"
}
