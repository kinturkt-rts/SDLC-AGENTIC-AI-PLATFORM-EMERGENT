# Per-app deploy root for target-app: team-faq-bot (generated by devops-agent).
# Deploy with: .\scripts\deploy-target-app.ps1 -Feature team-faq-bot
...
terraform {
  ...
  backend "s3" {
    ...
    key          = "dev/apps/team-faq-bot/terraform.tfstate"
    region       = "us-east-2"
    use_lockfile = true
  }
...
module "app" {
  ...
  enable_bedrock = true
  extra_env = {
    ADMIN_API_KEY = "dLAOz9jLnD0SL41iHymBDZfoDhhQ-q9y"
    API_KEY       = "lhL_HkgGvva6jlPHywQmlmFXT51HL15q"
  }
  has_database         = true
...
resource "aws_secretsmanager_secret" "db" {
  name                    = "sdlc/team-faq-bot/database-url"
...
output "app_url" {
  value = "http://${data.terraform_remote_state.shared.outputs.alb_dns_name}/team-faq-bot/"
}
