# Per-app deploy root for target-app: wellness-challenge-desk
# Auto-scaffolded by scripts/ensure-target-app-tf-root.py for destroy/redeploy.
# Deploy with: .\scripts\deploy-target-app.ps1 -Feature wellness-challenge-desk
...
terraform {
  ...
  backend "s3" {
    ...
    key          = "dev/apps/wellness-challenge-desk/terraform.tfstate"
    region       = "us-east-2"
    use_lockfile = true
  }
...
provider "aws" {
  ...
  default_tags {
    tags = {
      ...
      Environment = "dev"
      TargetApp   = "wellness-challenge-desk"
      ManagedBy   = "terraform"
    }
  }
}
...
module "app" {
  ...
  app_name    = "wellness-challenge-desk"
  environment = "dev"
...
resource "aws_secretsmanager_secret" "db" {
  name                    = "sdlc/wellness-challenge-desk/database-url"
...
output "app_url" {
  value = "http://${data.terraform_remote_state.shared.outputs.alb_dns_name}/wellness-challenge-desk/"
}
