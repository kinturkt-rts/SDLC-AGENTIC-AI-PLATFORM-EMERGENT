# Per-app deploy root for target-app: change-request-hub (generated by devops-agent).
# Deploy with: .\scripts\deploy-target-app.ps1 -Feature change-request-hub
...
terraform {
  ...
  backend "s3" {
    ...
    key          = "dev/apps/change-request-hub/terraform.tfstate"
    region       = "us-east-2"
    use_lockfile = true
  }
...
module "app" {
  ...
  enable_ui = true
  extra_env = {
    JWT_SECRET_KEY = "WDShUwuTqUSRofqwgcwseSI9Q9g3tDCx"
  }
  has_database         = true
...
resource "aws_secretsmanager_secret" "db" {
  name                    = "sdlc/change-request-hub/database-url"
...
output "app_url" {
  value = "http://${data.terraform_remote_state.shared.outputs.alb_dns_name}/change-request-hub/"
}
