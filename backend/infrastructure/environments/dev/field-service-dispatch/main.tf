# Per-app deploy root for target-app: field-service-dispatch (generated by devops-agent).
# Deploy with: .\scripts\deploy-target-app.ps1 -Feature field-service-dispatch
...
terraform {
  ...
  backend "s3" {
    ...
    key          = "dev/apps/field-service-dispatch/terraform.tfstate"
    region       = "us-east-2"
    use_lockfile = true
  }
...
module "app" {
  ...
  enable_bedrock = true
  extra_env = {
    API_KEY = "4mF9iPecxyhb_h4edZQPQyfLwwEfMsJo"
  }
  has_database         = true
...
resource "aws_secretsmanager_secret" "db" {
  name                    = "sdlc/field-service-dispatch/database-url"
...
output "app_url" {
  value = "http://${data.terraform_remote_state.shared.outputs.alb_dns_name}/field-service-dispatch/"
}
