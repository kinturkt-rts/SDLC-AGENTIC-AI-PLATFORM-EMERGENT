# Per-app deploy root for target-app: healthcare-clinic-bot (generated by devops-agent).
# Deploy with: .\scripts\deploy-target-app.ps1 -Feature healthcare-clinic-bot
...
terraform {
  ...
  backend "s3" {
    ...
    key          = "dev/apps/healthcare-clinic-bot/terraform.tfstate"
    region       = "us-east-2"
    use_lockfile = true
  }
...
provider "aws" {
  ...
  default_tags {
    tags = {
      ...
      Environment = "dev"
      TargetApp   = "healthcare-clinic-bot"
      ManagedBy   = "terraform"
    }
  }
}
...
module "app" {
  ...
  app_name    = "healthcare-clinic-bot"
  environment = "dev"
  ...
  enable_bedrock = true
  extra_env = {
    JWT_SECRET_KEY = "7nj59-z3G0yRwL9PIU9z3MLyUL2ItNIS"
  }
  has_database         = true
...
resource "aws_secretsmanager_secret" "db" {
  name                    = "sdlc/healthcare-clinic-bot/database-url"
...
output "app_url" {
  value = "http://${data.terraform_remote_state.shared.outputs.alb_dns_name}/healthcare-clinic-bot/"
}
