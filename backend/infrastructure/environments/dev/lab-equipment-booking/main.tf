# Per-app deploy root for target-app: lab-equipment-booking
# Auto-scaffolded by scripts/ensure-target-app-tf-root.py for destroy/redeploy.
# Deploy with: .\scripts\deploy-target-app.ps1 -Feature lab-equipment-booking
...
terraform {
  ...
  backend "s3" {
    ...
    key          = "dev/apps/lab-equipment-booking/terraform.tfstate"
    region       = "us-east-2"
    use_lockfile = true
  }
...
provider "aws" {
  ...
  default_tags {
    tags = {
      ...
      Environment = "dev"
      TargetApp   = "lab-equipment-booking"
      ManagedBy   = "terraform"
    }
  }
}
...
module "app" {
  ...
  app_name    = "lab-equipment-booking"
  environment = "dev"
...
resource "aws_secretsmanager_secret" "db" {
  name                    = "sdlc/lab-equipment-booking/database-url"
...
output "app_url" {
  value = "http://${data.terraform_remote_state.shared.outputs.alb_dns_name}/lab-equipment-booking/"
}
