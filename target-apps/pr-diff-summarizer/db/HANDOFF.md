# Database handoff — pr-diff-summarizer

_Generated 2026-06-18 19:54 UTC by database-agent._
...
| Item | Value |
|------|--------|
| Postgres schema | `pr_diff_summarizer` |
| Database | `(set POSTGRES_MCP_DATABASE)` |
| Endpoint | `(set POSTGRES_MCP_DB_ENDPOINT)` |
| SQL artifacts | `target-apps/pr-diff-summarizer/db/sql/` |
| RDS apply (last run) | not this session |
| Dev seed rows/table | 5–10 (see `*_seed.sql`) |

## SQL files (apply order)

1. `target-apps/pr-diff-summarizer/db/sql/001_create_reviews_table.sql`
2. `target-apps/pr-diff-summarizer/db/sql/002_create_api_keys_table.sql`
3. `target-apps/pr-diff-summarizer/db/sql/003_add_indexes.sql`
4. `target-apps/pr-diff-summarizer/db/sql/004_seed.sql`

**Connection:** load credentials from env/Key Vault (NFR-5). Use schema `pr_diff_summarizer` (`search_path` or qualified table names). Do not rely on unqualified `public` for app tables.

## ORM parity (required for live RDS)

- **ENUM `risk_band_enum`** → `sqlalchemy.Enum(..., name='risk_band_enum', schema='pr_diff_summarizer', create_type=False, native_enum=True)` + `.with_variant(String, 'sqlite')` (see `_template/app/models/pg_types.py`).
...
- **Driver/DSN** → `psycopg[binary]` in requirements; `.env.example`: `postgresql+psycopg://...?sslmode=require`; `POSTGRES_SCHEMA=pr_diff_summarizer` (set search_path in `database.py`, not copied from other apps).

## Developer-agent checklist

1. `dev_read_file` → `docs/design/pr-diff-summarizer.md`
2. `dev_read_file` → `target-apps/pr-diff-summarizer/db/sql/` migrations + seed
