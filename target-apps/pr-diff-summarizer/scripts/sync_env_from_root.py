"""Populate target-apps/pr-diff-summarizer/.env from the project root .env.

Reads POSTGRES_MCP_* vars from the project root .env, builds a URL-encoded
DATABASE_URL, and writes it (plus a random API_KEY when the placeholder is
still present) into target-apps/pr-diff-summarizer/.env.

Never prints credentials — only reports which lines were updated.
"""
...
def upsert_env_vars(app_env_path: Path, updates: dict[str, str]) -> list[str]:
    """Insert or replace KEY=VALUE lines in the app .env file."""
...
def update_app_env(app_env_path: Path, database_url: str, root_env: dict[str, str]) -> list[str]:
    ...
    for line in app_env_path.read_text(encoding="utf-8").splitlines():
        ...
        if stripped.startswith("API_KEY=") and PLACEHOLDER_API_KEY in stripped:
            new_lines.append(f"API_KEY=pr-diff-{secrets.token_hex(16)}")
