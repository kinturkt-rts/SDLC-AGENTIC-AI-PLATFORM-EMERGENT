"""Populate target-apps/training-compliance/.env from the project root .env."""
...
PLACEHOLDER_DSN_MARKERS = ("yourpassword", "username:password", "your-rds-host")
PLACEHOLDER_JWT = "change-me-to-a-strong-random-secret"
...
def update_app_env(app_env_path: Path, database_url: str) -> list[str]:
    ...
    for line in app_env_path.read_text(encoding="utf-8").splitlines():
        ...
        if stripped.startswith("JWT_SECRET_KEY=") and PLACEHOLDER_JWT in stripped:
            new_lines.append(f"JWT_SECRET_KEY=tc-{secrets.token_hex(24)}")
            updated.append("JWT_SECRET_KEY")
            continue
