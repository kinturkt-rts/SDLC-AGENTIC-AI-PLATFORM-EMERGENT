...
class Hold(Base):
    ...
    book_id = Column(
        PG_UUID(as_uuid=False).with_variant(String(36), "sqlite"),
        ForeignKey("library_catalog.books.id", ondelete="CASCADE"),
        nullable=False
    )
    member_id = Column(
        PG_UUID(as_uuid=False).with_variant(String(36), "sqlite"),
        ForeignKey("library_catalog.members.id", ondelete="CASCADE"),
        nullable=False
    )
    placed_at = Column(TimestampTZ, nullable=False, default=datetime.utcnow, server_default=func.now())
    fulfilled_at = Column(TimestampTZ, nullable=True)
