...
#
# Opt-in extras:
#   -WithJira -JiraProject SAAP          # after PRD: Epic + 5 stories (Atlassian MCP)
...
function Write-RunInstructions {
    ...
    Write-Host "  Open http://127.0.0.1:8000/docs  - auth per README (X-API-Key or JWT Bearer)"
}
...
if ($runQa) { Write-Host "  QA:      target-apps/$Feature/tests/" }
