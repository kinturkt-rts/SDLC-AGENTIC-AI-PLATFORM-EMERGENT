...
function InputRequirementsCard() {
  ...
  const handleSave = () => {
    ...
    toast.success('Requirements saved', { description: 'requirements_input.txt is ready for the pipeline.' });
  };

  const handleStart = () => {
    toast.success('SDLC Pipeline started', { description: 'Dispatching requirements_input.txt to Product Agent. (Mock — no backend execution.)' });
  };
  ...
  return (
    <Card className="overflow-hidden border-white/[0.06] bg-card/80">
      <div className="border-b border-white/[0.06] px-4 py-3">
        ...
        <p className="mt-0.5 text-[11px] text-muted-foreground">Upload or paste <code className="rounded bg-muted/50 px-1 py-0.5 font-mono text-foreground">requirements_input.txt</code> to start an agentic SDLC run.</p>
      </div>
      ...
      <div className="flex flex-col gap-4 p-4 lg:flex-row">
        ...
        <div className="flex flex-col gap-3 lg:w-48">
          ...
          <div className="space-y-1.5 text-xs text-muted-foreground">
            <p className="font-mono">requirements_input.txt</p>
            {lastSaved && <p>Saved at {lastSaved}</p>}
        ...
        <div className="min-w-0 flex-1 space-y-3">
          <Textarea
            ...
            placeholder="Paste your requirements here, or upload a file..."
            rows={6}
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <Save className="h-3.5 w-3.5" /> Save Input
            </Button>
            <Button
              size="sm"
            ...
            >
              <PlayCircle className="h-3.5 w-3.5" /> Start SDLC Pipeline
            </Button>
            {status === 'missing' && (
              <p className="text-[11px] text-red-400/80">Add requirements_input.txt before starting the pipeline.</p>
            )}
            {status === 'ready' && (
