...
### Double-Depends — don't use Annotated-with-Depends as a type AND `= Depends(...)` default

When `app/dependencies.py` exposes a shorthand like
`AuthUser = Annotated[CurrentUser, Depends(get_current_user)]`, the `Depends`
is already embedded in the type. Using that type as a parameter annotation
AND providing `= Depends(...)` as the default value triggers:
...
```python
# wrong — Depends in Annotated + Depends in default = double-Depends
def list_audits(
    db: DbSession,
    current_user: AuthUser = Depends(require_auditor),  # ← fails
): ...

# right (option 1) — use the bare class with `Depends(...)` default
def list_audits(
    db: DbSession,
    current_user: CurrentUser = Depends(require_auditor),
): ...

# right (option 2) — define a second Annotated shorthand for the RBAC dep
